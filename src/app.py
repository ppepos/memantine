...
class User(UserMixin, db.Document):
    ...
    total_spent = db.DecimalField(required=False, min_value=0, precision=2)
    totals_spent = db.DictField(required=True)
...
# TODO Remove all DecimalFields as they cannot be inserted into a mongo dict
class Spending(db.Document):
    ...
    for_who = db.StringField(max_length=32, required=False)
...
class SpendingsForm(Form):
    ...
    for_who = SelectField('For',
                          default='all',
                          choices=[(u.username, u.display_name) for u in
                                   User.objects] + [('all', 'Everyone')],
                          filters=[lambda x: 'all' if x == 'None' else x])
...
def index():
    ...
    for user in users:
        if user.get_id() == current_user_id:
            current_user_balance = user.total_spent - \
                (total_spent_by_all_users / users.count())

        # item, amount, date of last 3 spendings of user
        recent_spendings[user.username] = \
            Spending.objects(spender=user.username).only("item",
                                                         "amount",
                                                         "date"
                                                         ).order_by("-date")[:3]
...
def account_settings():
    ...
    if form.validate_on_submit():
        ...
        if new_display or new_pass:
            ...
            flash('Successfully updated your settings')
...
def spend():
    ...
    if form.validate_on_submit():
        ...
        spending.for_who = form.for_who.data
        spending.save()
        user_id = current_user.get_id()
        user = load_user(user_id)

        # Convert from old-style user schema
        if user.total_spent:
            user.totals_spent = {}
            user.totals_spent['all'] = int(user.total_spent * 100)
            del user.total_spent

        # Initialize the dict entry if needed
        if not user.totals_spent.get(spending.for_who, None):
            user.totals_spent[spending.for_who] = 0
        user.totals_spent[spending.for_who] += int(spending.amount * 100)
